...
COLLECTION="$ROOT_DIR/postman/multitenant-restaurant-api.postman_collection.json"
ENVIRONMENT="$ROOT_DIR/postman/multitenant-restaurant-api.environment.json"
EXTRA_ARGS=()
NETWORK_ARG=()
...
DEFAULT_BASE_URL=""
APP_CONTAINER="${APP_CONTAINER_NAME:-restaurant-hub-api}"
...
if docker inspect "$APP_CONTAINER" >/dev/null 2>&1; then
  ...
  if [[ -n "$NETWORK_NAME" ]]; then
    ...
    DEFAULT_BASE_URL="http://$APP_CONTAINER:8080"
  fi
fi
...
docker run --rm ${PLATFORM_ARG+"${PLATFORM_ARG[@]}"} ${NETWORK_ARG+"${NETWORK_ARG[@]}"} -v "$ROOT_DIR:/etc/newman" postman/newman:5-alpine \
  run /etc/newman/postman/multitenant-restaurant-api.postman_collection.json \
  -e /etc/newman/postman/multitenant-restaurant-api.environment.json \
  --reporters cli \
